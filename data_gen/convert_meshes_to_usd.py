...
parser.add_argument("input_dir", type=str, help="The directory path to the input mesh file.")
parser.add_argument("output_dir", type=str, help="The directory path to store the USD file.")
parser.add_argument(
    "--make-instanceable",
    action="store_true",
    default=False,
...
def main():
    ...
    mesh_dir = Path(args_cli.input_dir)
    mesh_paths = list(mesh_dir.glob("*.obj"))
    mesh_paths += list(mesh_dir.glob("*.stl"))
    mesh_paths += list(mesh_dir.glob("*.fbx"))
    ...
    for mesh_path in mesh_paths:
        ...
        dest_path = Path(args_cli.output_dir) / os.path.basename(mesh_path)[:-4] / (os.path.basename(mesh_path)[:-4] + ".usd")
