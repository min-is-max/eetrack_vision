...
@TESTERS.register_module()
class BsplineTester(TesterBase):
    def __init__(
        self,
        knots_from=["uniform"],
        spline_degree=None,
    ...
    def test(self):
        ...
        for i, input_dict in enumerate(self.test_loader):
            ...
            if self.vis:
                ...
                # edge_gt_spl = edge_gt_bspline(u_eval)
                # edge_gt_spline = get_point_cloud(edge_gt_spl[0], color=np.array([[1, 0, 0]]), verbose=False)[0]
                # o3d.visualization.draw_geometries([scene_pcd, edge_gt_pcd, edge_gt_spline])
...
class SimpleSemSegTester(TesterBase):
    def test(self):
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            coord_pred_pcd, coord_pred_ids = edge_pred_pcd.remove_radius_outlier(nb_points=10,radius=0.005) # 10. 0.005
            ...
            # intrinsic for saved output
            # K = np.array([
            #     [1.056968017578125000e+03, 0.000000000000000000e+00, 1.121511474609375000e+03,]
            #     [0.000000000000000000e+00, 1.056968017578125000e+03, 6.245552978515625000e+02,]
            ...
            rgb, depth = image_from_point_clouds([scene_pcd, edge_pred_pcd, spl_pcd], K, np.eye(4),1000, 2048)
            Image.fromarray(rgb).save(os.path.join(save_path, f"{data_name}.png"))
            print(f"Image saved to: {os.path.join(save_path, f'{data_name}.png')}")
...
# ----------------------------- #
# Deploy-only simple tester     #
# ----------------------------- #
import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt

@TESTERS.register_module(name="DeployPreviewTester")
class DeployPreviewTester(TesterBase):
    def _save_preview(self, coord_np, pred_mask_np, out_png):
        """
        coord_np: (N,3) float32
        pred_mask_np: (N,) int {0,1}
        out_png: str
        """
        if coord_np.ndim != 2 or coord_np.shape[1] != 3:
            return
        # PCA 투영(2D)
        X = coord_np - coord_np.mean(axis=0, keepdims=True)
        # SVD로 주성분 2개
        try:
        ...
        Q = (P - [xmin, ymin]) / [w, h]
        W, H = 1024, 1024
        px = np.clip((Q[:, 0] * (W - 1)).astype(np.int32), 0, W - 1)
        py = np.clip((Q[:, 1] * (H - 1)).astype(np.int32), 0, H - 1)
        # 그리기: 배경 흰색, 점은 회색, 엣지는 빨강
        img = np.ones((H, W, 3), dtype=np.uint8) * 255
        # 점 간소화(밀집일 때 과도 덮어쓰기 방지): 간단히 1픽셀 찍기
        img[py, px] = np.array([200, 200, 200], dtype=np.uint8)
        edge_idx = pred_mask_np.astype(bool)
        if edge_idx.any():
            img[py[edge_idx], px[edge_idx]] = np.array([0, 0, 255], dtype=np.uint8)  # BGR=red
        # matplotlib 없이 저장
        import imageio
    ...
    def test(self):
        assert self.test_loader.batch_size == 1
        logger = get_root_logger()
        logger.info(">>>>>>>>>>>>>>>> Start Deploy Preview (no metrics) >>>>>>>>>>>>>>>>")
        self.model.eval()
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            pred = out["seg_logits"].max(1)[1]  # (N,)
            pred_np = pred.detach().cpu().numpy()

            # 기본 산출물 저장
            np.save(os.path.join(save_path, f"pred_{idx:06d}.npy"), pred_np)

            # 간단 프리뷰 (Open3D 불필요, 2D PCA 투영)
            coord = data_dict["coord"].detach().cpu().numpy()  # (N,3)
            try:
                self._save_preview(
                    coord_np=coord,
                    pred_mask_np=(pred_np == 1).astype(np.uint8),
                    out_png=os.path.join(save_path, f"preview_{idx:06d}.png"),
                )
            except Exception as e:
                if self.verbose:
                    print("preview save failed:", repr(e))

            logger.info(f"Saved: pred_{idx:06d}.npy, preview_{idx:06d}.png")
